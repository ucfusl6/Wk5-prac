...
### breaks=breaks 自然斷點--> let the system decide the break 
### Lengend, scale_bar, layout (explainary things) cannot overlap the main element of the map
```{r}
...
# set the breaks
# for our mapped data
breaks = c(0, 5, 12, 26, 57, 550)
# 自然斷點 --> do not need to specify the breaks, just use breaks=breaks 
...
legend <- tm_shape(Hotels) +
    ...
    tm_compass(north=0, position=c(0.8,0.6))+
    tm_layout(legend.only = TRUE, legend.position=c(0.2,0.24),asp=0.1)+
...
main <- tm_shape(Airbnb, bbbox = Londonbb) + 
  tm_polygons("Accomodation count",
              breaks=breaks, 
              palette="PuBu")+
  tm_scale_bar(position = c("left", "bottom"), text.size = .4)+ ####Can change this
  tm_layout(legend.position = c("right","top"), 
            legend.text.size=.4, ####Can change this
            legend.title.size = 0.9, ####Can change this
            frame=FALSE)+
  tm_credits("(c) OpenStreetMap contrbutors and Air b n b", position=c(0.0,0.0))+
